...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            ##################################################
            # Caption / Precomputed Embeddings
            ##################################################
            ...
            if self.cached_text_embeddings is None:
                ...
                if throw_one(self.caption_dropout_rate):
                    # Check for underscore variant first, then fallback
                    empty_key = f"{txt_name}_empty_embedding"
                    if empty_key not in self.cached_text_embeddings:
                        empty_key = f"{txt_name}empty_embedding"
                    
                    emb = self.cached_text_embeddings[empty_key]
                else:
                    emb = self.cached_text_embeddings[txt_name]

                # Safeguard against None masks
                prompt_embeds = emb["prompt_embeds"]
                prompt_embeds_mask = (
                    emb["prompt_embeds_mask"] 
                    if emb["prompt_embeds_mask"] is not None 
                    else torch.tensor([])
                )
