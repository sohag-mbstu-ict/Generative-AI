...
class GemmaFineTuner:
    """
    A class for fine-tuning the Gemma model using Unsloth with LoRA adapters and Alpaca-style dataset.
    """
    ...
    def prepare_dataset(self):
        """Load and format the Alpaca-style dataset for fine-tuning."""
        ...
        def formatting_prompts_func(examples):
            ...
            for instruction, input_text, output in zip(instructions, inputs, outputs):
                text = alpaca_prompt.format(instruction, input_text, output) + self.EOS_TOKEN
                texts.append(text)
            return {"text": texts}

        dataset = load_dataset(self.dataset_path, split="train")
        print("Original Dataset Loaded:", dataset)
...
if __name__ == "__main__":
    ...
    dataset_path = "/home/gflmltpc/Projects/Gen_AI/Dataset/alpaca/"
    output_dir = "/home/gflmltpc/Projects/Gen_AI/LLM_Fine_Tuning/Alpaca-Format/trained_gemma_7b_26_july_25"

    fine_tuner = GemmaFineTuner(base_model_path, dataset_path, output_dir)
    fine_tuner.load_model()
    fine_tuner.apply_lora(r=16)
    dataset = fine_tuner.prepare_dataset()
    fine_tuner.configure_trainer(dataset, epochs=21)
    fine_tuner.train()
