...
print("Cached memory (bytes):", torch.cuda.memory_reserved())

import os
os.environ["UNSLOTH_DISABLE_STATS"] = "1"  # Must be first
import wandb
import unsloth
...
class Qwen3InstructFineTuner:
    ...
    def init_wandb(self):
        self.wandb_run = wandb.init(
            project="gemma-3-4b-instruct",
            name="early-stopping-lora-run",
            config={
                ...
                "dataset": os.path.basename(self.json_path),
            },
        )
    ...
    # -------------------------------
    # Convert ChatML → ShareGPT format
    # -------------------------------
    @staticmethod
    ...
    def setup_trainer(self):
        ...
        if len(self.dataset) > 1:
            split = self.dataset.train_test_split(test_size=0.1, seed=self.seed)
